...
public class IpGroupUserDAO extends HibernateDaoSupport {
	...
	public void deleteByGroupId(Long id) {
		...
		try {
			Query query = getSession().getNamedQuery("deleteGUByGroupId");
			query.setLong("id", id);
			query.executeUpdate();
		} catch (RuntimeException re) {
			log.error("attach failed", re);
	...
	public List fetchByGroupId(Long id) {
		...
		try {
			Query query = getSession().getNamedQuery("fetchGUByGroupId");
			query.setLong("id", id);
			List ret = query.list();
	...
	public List fetchByUserId(Long id) {
		log.debug("Fetching Group Users By Id : " + id);
		try {
			Query query = getSession().getNamedQuery("fetchGUByUserId");
			query.setLong("id", id);
			List ret = query.list();
			for (Object object : ret) {
				IpGroupUser gu = (IpGroupUser) object;
