...
public class MySqlAuthenticator implements Authenticator {
	...
	private int authenticate(long companyId, String authPrinciple, String password, int authType) {
		...
		case AUTH_BY_SCREEN_NAME:
			WebClient loginClient = RESTServiceHelper.createCustomClient("http://" + BUNDLE.getString("ws.host") + ":" + BUNDLE.getString("ws.port") + "/ip-ws/ip/as/user/login/" + authPrinciple + "/" + Base64.encodeBase64URLSafeString(DigestUtils.md5(password.getBytes())));
